...
let cachedKnowledgeText = null;
let cachedKnowledgeTextMtimeMs = null;

export async function readKnowledgeText() {
  const knowledge = await loadKnowledgeDocument();
...
export async function writeKnowledgeText(text) {
  ...
  const stats = await fs.stat(KNOWLEDGE_FILE);
  cachedKnowledgeText = text.trim() ? text : "";
  cachedKnowledgeTextMtimeMs = stats.mtimeMs;
}
...
export async function loadKnowledgeDocument() {
  ...
  if (
    ...
    return {
      text: cachedKnowledgeText,
      mtimeMs: cachedKnowledgeTextMtimeMs,
    };
  }

  const text = await fs.readFile(KNOWLEDGE_FILE, "utf8");
  cachedKnowledgeText = text;
  cachedKnowledgeTextMtimeMs = stats.mtimeMs;
  return {
    text,
...
export async function loadEmbeddingStore() {
  ...
  try {
    ...
    const store = {
      ...
      embedModelPath: data.embedModelPath || null,
...
export function isEmbeddingStoreCompatible(
  store,
  ...
  if (!store.embedModel || !embedModel || store.embedModel !== embedModel) {
    return false;
  }

  if (store.embedModelPath && embedModelPath && store.embedModelPath !== embedModelPath) {
    return false;
  }
